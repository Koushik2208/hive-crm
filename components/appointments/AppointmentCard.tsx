...
export default function AppointmentCard({ appointment }: AppointmentCardProps) {
  ...
  // We'll trust mock bounds for this foundation.
  ...
  const height = appointment.service.duration_mins * pixelsPerMinute;
  ...
  return (
    <div
      className={`absolute shadow-[var(--shadow-ambient)] rounded-2xl bg-white p-4 border-l-4 ${borderClass} group hover:scale-[1.02] transition-transform cursor-pointer overflow-hidden z-10`}
      style={{
        top: `${topPosition}px`,
        height: `${height}px`,
      ...
      {height >= 72 && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
          <div className="flex items-center gap-1 text-[0.65rem] text-outline font-medium">
            <Clock size={14} />
            {formatTime(appointment.starts_at)} ({appointment.service.duration_mins} min)
          </div>
          <div className="flex items-center gap-1 text-[0.65rem] text-outline font-medium">
            <User size={14} />
